...
$$\textit{log(cena)} = \beta_{0} +  \beta_{1}pokoje  + \beta_{2}metry + \beta_{3}metry^{2} + \beta_{4}mezon + \beta_{5}kk + \beta_{6}panel + \beta_{7}terasa/balkon + \beta_{8}novostavba + \varepsilon.$$
...
```{r, results='asis'}
formula <- as.formula(log(price) ~ Rooms + Meters + I(Meters^2) + Mezone + KK + panel + balcony_or_terrase + novostavba)
...
model_kmeans <- lm(log(price) ~ Rooms + Meters + I(Meters^2) + Mezone + KK + panel + balcony_or_terrase + novostavba + factor(KMEAN), df)

model_quant_kmean <- rq(log(price) ~ Rooms + Meters + I(Meters^2) + Mezone + KK + panel + balcony_or_terrase + novostavba + factor(KMEAN), data = df)
